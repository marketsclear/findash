...
- ETF adapters live in `lib/sources/etf/`, one file per issuer, sharing the `FundAdapter` interface. Each returns issuer snapshots (date, shares, nav, eth, aum); flows are derived in `lib/etf-flows.ts`. Verified against Farside: ETHA/ETHE/ETH need `sharesLag: 1`, TETH `sharesLag: 0`. Check `scripts/etf-probe.ts <TICKER>` after touching an adapter. Fidelity and Invesco go through `scripts/impersonate_fetch.py` (needs `pip install curl_cffi`); never "simplify" them back to plain fetch, Akamai returns 403/406.
